...
const notificationService = new NotificationService(new UserService().getAllUsers());
        ...
        case "bug":
            ...
            try {
                if (validTransitions[previousStatus].indexOf(task.getStatus()) !== -1) {
                    ...
                    notificationService.notifyAdmins(`Bug task ${task.getId()} foi processada`);
                    previousStatus = task.getStatus();
                }
            }
            catch (error) {
                console.error(`Invalid status transition from ${previousStatus} to ${task.getStatus()}`);
            }
            break;
        case "feature":
