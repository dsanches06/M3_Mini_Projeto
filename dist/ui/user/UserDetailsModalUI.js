...
function modalUserDetail(user) {
    ...
    detailStatus.innerHTML = `<strong>Status:</strong> ${user.isActive() ? "Ativo" : "Inativo"}`;
...
function modalUserTask(user) {
    ...
    user.getTasks().forEach((task) => {
        ...
        li.textContent = `${task.getTitle()} - ${task.getCompleted() ? "Concluído" : "Pendente"} - ${task.getTaskCategory()} - ${task.getCompletedDate()
