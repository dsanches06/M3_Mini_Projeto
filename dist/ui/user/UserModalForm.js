 ...
 */
function setupFormLogic(UserService, form, fields, errors, modal) {
    form.onsubmit = (e) => {
        e.preventDefault();
        // Reset de estados
        ...
        if (isValid) {
            ...
            let newId = getLastId(UserService.users) + 1;
            ...
            if (user && user.getName()) {
                showInfoBanner(`${user.getName()} foi adicionado com sucesso.`, "info-banner");
            }
            else {
                showInfoBanner(`ERRO: ${fields.name.value} não foi adicionado.`, "error-banner");
            }
            //mostra todos os utilizadores
            renderUsers(UserService, UserService.users);
            // atualizar contadores
            showUsersCounters(UserService.users);
            modal.remove();
        }
        else {
 ...
 */
export function renderUserModal(UserService) {
    ...
    setupFormLogic(UserService, form, { name: nameData.input, email: emailData.input }, {
