 ...
 */
export default function createAndAppendTaskForm(containerId, user, UserService) {
    ...
    form.addEventListener("submit", (e) => handleFormSubmit(e, user, UserService));
}
/**
 * 2. Função para processar os dados
 */
const handleFormSubmit = (event, user, UserService) => {
    ...
    const category = form.querySelector("#taskCategory")
        .value;
    if (title.trim()) {
        const allTasks = [];
        UserService.users.forEach((u) => allTasks.push(...u.tasks));
